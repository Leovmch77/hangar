...
const b = (key: string, extra: Partial<UsoBucket> = {}): UsoBucket => ({
  ...zeroUso(key), sessions: 1, chamadas: 10, ctx_chars: 400, ctx_tokens_est: 100,
  cost: 1, cost_input: 1, ...extra,
});
...
describe('mergeUso', () => {
  it('soma por chave entre máquinas e carimba o corte por servidor', () => {
    const m = mergeUso([
      ...
      { id: 'b', label: 'B', report: { totals: b('totals', { cost: 3, cost_input: 2, cost_output: 1 }), by_skill: [b('orquestrar'), b('ecc:x', { plugin: 'ecc' })], applied: { period: '30d' }, usd_brl: 5 } },
    ], '30d');
    expect(m.partial).toBe(false);
    expect(m.report.totals.chamadas).toBe(20);
    expect(m.report.totals.cost).toBe(4);
    expect([m.report.totals.cost_input, m.report.totals.cost_output]).toEqual([3, 1]);
